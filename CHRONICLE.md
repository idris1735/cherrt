...
- **P2 tail — CLEARED (same day):** (1) extended the universal escape hatch to also cover `session.onboarding`, so cancel/exit/quit/menu/start over now bail out of ALL onboarding-based flows (signup, post-approval-setup, assign-role) uniformly — menu/start-over renders the real menu; also broadened assign-role's own cancel for direct callers. (2) `give.ts` now accepts an inline correction: after a mis-parsed seeded amount, sending a new number re-sets it (re-render) instead of forcing cancel+restart; "ok"/type proceeds. +6 regression tests. Nothing embarrassing left from the sweep.
