...
### Chatbot personality (2026-07-22)
`src/lib/services/agent/persona.ts` — `AGENT_PERSONA` gives Chertt a deliberate, consistent character (replaces the thin inline system prompt in runtime.ts): warm unflappable church-secretary voice, Nigerian English + Pidgin, WhatsApp-brief, faith-comfortable but never preachy, tone that adapts (everyday=quick, tender=gentle no-emoji, finance=precise). Safety-critical behaviours are baked in and **test-locked** (`persona.test.ts`): honesty (never invent), confidentiality (never share one person's prayer/giving/pastoral with another), confirmation before consequential actions, and **crisis handling** (stay calm, don't counsel, urge 112/emergency help, log via pastoral-care category 'crisis' for urgent pastor follow-up). 284 tests pass. Personality is a single tunable module — easy to adjust the voice or make it per-church later.
