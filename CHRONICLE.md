...
- **Remaining for Slice 2 — Plan 4:** tiered access (block money/invite/broadcast for non-`approved` churches) + rewire the WhatsApp "set up my church" flow to issue a KYC token & send the `/onboard/[token]` link (until then the form has no in-product entry point — reachable only via a manually created token).
