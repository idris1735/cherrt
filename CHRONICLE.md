...
Live via the agent: giving summary + record giving, prayer requests (capture + list), first-timers (capture + list), pastoral care (capture), child check-in (check-in + pickup verify + gated release), **event registration (list + register), department joining (list + apply)**, members (list + assign role), facility issues (report + list), documents (gated draft). Still on the single-shot creator / not yet agent-native: announcements (broadcast), bereavement/marriage/baptism/discipleship journeys, real giving *payments*.
